...
export const ANDELA_STAGES: TestStage[] = [
  ...
  },
  {
    ...
    type: "automated",
    duration: "~90 minutes",
    format: "Coding tasks + deep-knowledge MCQ (System Design, RAG, FDE scenarios, PR review) — Andela's Qualified platform",
    passMark: "Passing score varies by role; includes code quality evaluation",
    whatToExpect: [
      "Proctored — webcam required. No human interviewer.",
      "Coding section: implement a function to pass a given test suite, fix a bug. Read the failing tests FIRST — they are the spec.",
      "MCQ section: real-world scenario questions covering System Design trade-offs, RAG edge cases (recall vs precision, hallucination), complex FDE client situations, and PR review (spotting bugs/smells in a code diff).",
      "RAG is heavily weighted — expect questions on chunking strategies, retrieval failure modes, reranking, and RAGAS evaluation metrics.",
    ],
    keyTip: "Do not rush past the MCQ — it carries as much weight as the coding. For RAG questions, know the difference between precision (relevant docs returned) and recall (relevant docs missed), and when hallucination still happens even with good retrieval.",
    color: "#0d9488",
  },
  {
    id: "andela-a3-screen",
