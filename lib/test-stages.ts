...
export const ANDELA_STAGES: TestStage[] = [
  ...
  },
  {
    ...
    type: "automated",
    duration: "~90 minutes",
    format: "Woven portal — coding tasks (backdated invoice/data challenge) + PR review scenario + RAG/System Design MCQs",
    passMark: "Passing score varies by role; submissions are stored and used as reference in all subsequent live stages",
    whatToExpect: [
      "Proctored via webcam — Woven portal. No human interviewer but engineers review your exact code later.",
      "Code challenge: data transformation tasks (e.g., backdated invoice calculation, billing proration). Graded on edge-case logic, float precision (use integer cents), timezone handling (UTC), and Big-O efficiency (O(N) lookup maps over O(N²) nested loops).",
      "PR review scenario: given a messy codebase diff, identify async flaws (unhandled rejections), security issues (SQL injection, hardcoded credentials), and race conditions. Structure feedback as Risk → Impact → Resolution.",
      "MCQ section: RAG (chunking strategies, hybrid search, PII governance, hallucination), System Design trade-offs, and complex FDE client situations.",
    ],
    keyTip: "Your Woven submissions ARE the reference for live stages — engineers will ask you to modify, expand, and defend every line. Write clean, commented, production-quality code, not just passing code.",
    color: "#0d9488",
  },
  {
    id: "andela-a3-screen",
